...
async def handle(update: Update, context: CallbackContext):
    ...
    except Exception as e: logger.error(f'{__name__}: <api_handle> {config.lang["errores"]["error"][config.pred_lang]}: {config.lang["errores"]["menu_modes_not_ready_yet"][config.pred_lang]} {e}')
